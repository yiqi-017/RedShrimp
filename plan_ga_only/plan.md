...
### 5.3 应补充到 constraints 的平台实测信息
若 GA 已真实打开小红书网页端发布页，则不应只传抽象的 `content_type=image_text`，还应把页面上可观测到的发布能力与限制写入 `constraints` 或 `context`，例如：

- 图文发布页存在 `上传图片` 与 `文字配图` 两种流
- 单张图片最大 `32MB`
- 推荐格式 `png / jpg / jpeg / webp`
- 明确 `不支持 gif、live 及其转化后的图片`
- 比例推荐在 `3:4` 至 `2:1`
- 推荐分辨率不低于 `720*960`
- `超过1280P的图片用网页端上传画质更清晰`
- 文字配图编辑区实测为 `ProseMirror`
- 输入后页面会出现 `自动保存于 HH:MM`
- `生成图片 / 下一步 / 发布` 等按钮在执行侧可能需要 CDP 真实点击，而非普通 JS click

这类约束会直接影响 RedShrimp 的内容建议：例如不要依赖 GIF / Live Photo；若方案要走文字配图流，应允许“先写文案，再生成图片”；若方案依赖高清细节图，应优先走网页端上传链路。
