...
- 单帖详情抓取：能在小红书真实页面里定位目标卡片，进入详情后抓取时间文案（缺少发布时间时可将编辑时间暂作发布时间处理）、点赞数、收藏数、评论数与评论明细。
...
4. 数据目录约定：`data/TXT/`（人工查看）、`data/JSON/`（结构化留存）、`data/CSV/`（表格分析/可视化/LightGBM）；其中 `data/` 始终代表**最新一轮抓取结果**，新抓取会覆盖为最新内容；每轮最终结果还应归档到 `history/run1/data/`、`history/run2/data/`、`history/run3/data/` ...
5. 单帖详情抓取经验参考：`C:/Users/Hp/Desktop/y4s2/lab/GenericAgent/memory/xhs_detail_fetch_sop.md`
6. 最近一次实时趋势观察：`data/TXT/trends_data.txt` / `data/JSON/trends_data.json` / `data/CSV/trends_data.csv`
7. 本次/历次抓到的全部帖子：`data/TXT/all_captured_posts.txt` / `data/JSON/all_captured_posts.json` / `data/CSV/all_captured_posts.csv`
8. 最近热门帖子（1 小时）：`data/TXT/recent_hot_1h.txt` / `data/JSON/recent_hot_1h.json` / `data/CSV/recent_hot_1h.csv`
9. 最近热门帖子（3 小时）：`data/TXT/recent_hot_3h.txt` / `data/JSON/recent_hot_3h.json` / `data/CSV/recent_hot_3h.csv`
10. 最近热门帖子（6 小时）：`data/TXT/recent_hot_6h.txt` / `data/JSON/recent_hot_6h.json` / `data/CSV/recent_hot_6h.csv`
11. 最近热门帖子（12 小时）：`data/TXT/recent_hot_12h.txt` / `data/JSON/recent_hot_12h.json` / `data/CSV/recent_hot_12h.csv`
12. 最近热门帖子（1 天）：`data/TXT/recent_hot_1d.txt` / `data/JSON/recent_hot_1d.json` / `data/CSV/recent_hot_1d.csv`
13. 不限时很火帖子档案：`data/TXT/high_ces_archive.txt` / `data/JSON/high_ces_archive.json` / `data/CSV/high_ces_archive.csv`
...
## 动态案例库扩充与结果归档
### A. data 自动归档规则
实时趋势观察成功后，抓取产生的所有数据统一先写入 `data/`，并按格式拆分到三个目录：
- `data/TXT/`：给人直接看，适合快速翻阅、人工复盘。
- `data/JSON/`：给结构化留存，适合字段稳定、嵌套信息、程序二次处理。
- `data/CSV/`：给表格分析、数据可视化、特征工程与 LightGBM 训练。

补充约定：
- `data/` 始终表示**当前最新一轮抓取结果**，方便下游流程直接读取；
- 当发起新一轮抓取时，`RedShrimp/data/` 可以被覆盖为新的结果，不要求长期保留旧轮次；
- 但每一轮抓取在结束时，都必须把该轮完整结果额外归档到 `history/runN/data/`（如 `history/run1/data/`、`history/run2/data/`、`history/run3/data/` ...）；
- `history/runN/data/` 是按轮次保留的历史快照，供回溯、对比、复盘和排查使用；
- 若需要人工快速查看“最新结果”，默认看 `data/`；若需要查看历史某次结果，再进入对应的 `history/runN/data/`。
...
### 4A. 单帖详情页时间/赞藏评抓取经验流程
当用户给的是明确笔记、目标卡片，或明确要求“抓发布时间/编辑时间、点赞数、收藏数、评论数”时，按以下顺序执行：
1. **优先从站内真实卡片进入详情**：不要把直接裸开 `/explore/<noteId>` 作为首选；优先在推荐流、搜索结果页、作者主页或列表页定位卡片后点击进入详情，稳定性更高。
2. **若首屏找不到目标卡片，不要盲点**：先用 `web_scan` 看当前页是否真有该卡；若推荐流首屏没有，就改为从页面状态、已有 `detailUrl`、搜索结果或作者页重新定位。
3. **进入详情后先确认 noteId**：核对当前 URL 中的 `noteId`，再定向读取 `window.__INITIAL_STATE__.note.noteDetailMap[noteId]`，不要直接返回整棵 `__INITIAL_STATE__`，避免循环引用或超大对象导致失败。
4. **优先读状态对象里的稳定分支**：
   - `...note`：标题、正文、基础信息；
   - `...comments.list[*]`：评论、评论时间、点赞、子评论数；
   - 若能读到帖子级互动字段，优先用状态对象结果。
5. **帖子级时间优先级**：
   - 先找详情页底部可见时间文案，如“发布于 …”或“编辑于 …”；
   - 若只抓到“编辑于 …”，且任务允许，可暂按发布时间处理，但要在结果里注明是编辑时间；
   - 正文里的事件时间（如“4月15日凌晨”）不能当平台发布时间。
6. **点赞/收藏/评论总数优先抓详情底部互动区**：若状态对象未直出总数，改抓详情页主帖互动栏的可见数字；实测可从详情底部输入框/互动栏附近一起抓到主帖的点赞、收藏、评论总数。
7. **避免把评论区数字误当主帖总数**：评论列表中每条评论也有点赞/回复数字，只有与主帖底部互动栏同一区域、同一行出现的数字，才可当作主帖总点赞/总收藏/总评论。
8. **评论总数可双重校验**：
   - 详情页通常可见“共 xxx 条评论”；
   - 底部互动栏也常有评论总数；
   - 两者一致时优先作为已核实结果。
9. **推荐的结果字段**：`noteId`、`title`、`detail_url`、`time_text`、`likedCount`、`collectedCount`、`commentCount`、`capture_status`、`comments_sample`。
10. **推荐的 `capture_status` 写法**：
   - `ok_detail_dom_verified`：详情 DOM 已核验到时间与互动总数；
   - `ok_state_only_partial`：状态对象只有部分字段；
   - `needs_reopen_detail`：当前页不是详情页，需重进。
11. **已验证样例**：某实测笔记在详情页抓到 `time_text = 编辑于 昨天 14:25 广东`，且在主帖底部互动区抓到 `likedCount = 3769`、`collectedCount = 409`、`commentCount = 685`；评论区文案也出现“共 685 条评论”，可做交叉验证。
...
| 推荐流首屏没有目标卡片 | 不要盲点；先 `web_scan` 确认当前页，再改从页面状态、搜索结果、作者页或已知 `detailUrl` 重新进入详情。 |
| 直接裸开 `/explore/<noteId>` 不稳定或被拦 | 改为从站内 feed/列表页点击卡片进入详情，再读 `noteDetailMap[noteId]` 与详情 DOM。 |
| 页面信息只暴露部分字段 | 保留已拿到的字段，写入 `capture_status`；帖子级时间/总互动数缺失时，优先补抓详情 DOM，不要硬编。 |
